...
class Orders extends Component {
    render() {
        ...
        const childrenWithProps = React.Children.map( children, child => React.cloneElement( child, { routerProps: routerProps } ) )
        return (
            <div>
                ...
                <div className="item__grid">
                    <div className='item__row item__row--header'>
                        <span className="item__detail item__detail--icon"></span>
                    ...
                    { orders.map( order => {
                        return (
                            <Link to={'/orders/' + order.Order_Id} key={order.Order_Id} className='item__row'>
                                <span className="item__detail item__detail--icon"><span className={ order.Completed ? 'icon--check' : 'icon--bang' }></span></span>
                                ...
                                <span className="item__detail">${ ( order.Marketplace_Fee + order.Transaction_Fee + order.Shipping ).toFixed( 2 ) }</span>
                                <span className="item__detail">${ ( order.Profit_calc ).toFixed( 2 ) }</span>
                                <span className="item__detail item__detail--date">{ util.formatDate(order.Sold_Date) }</span>
                            </Link>
                        )
                    })}
                </div>
